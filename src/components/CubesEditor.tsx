...
// Colores iniciales para los cubos
...
function Cube({ 
  ...
  // Maneja el cambio de transformación con restricciones
...
function Scene({ 
  ...
  // Maneja la selección de cubo, aplicando cambios antes de cambiar la selección
...
export default function CubesEditor() {
  ...
  const handleAddCube = () => {
    const newCube = {
      ...
      color: initialColors[Math.floor(Math.random() * initialColors.length)],
      position: [
        clamp(Math.random() * 10 - 5, POSITION_LIMITS.x.min, POSITION_LIMITS.x.max),
        clamp(0.5, POSITION_LIMITS.y.min, POSITION_LIMITS.y.max),
        clamp(Math.random() * 10 - 5, POSITION_LIMITS.z.min, POSITION_LIMITS.z.max)
      ] as [number, number, number]
    }
            ...
            <div className="space-y-1">
              <label className="flex items-center">
                ...
                <input
                  ...
                  onChange={(e) => setEditDimensions(prev => ({ ...prev, width: parseFloat(e.target.value) || 0 }))}
              ...
              <label className="flex items-center">
                ...
                <input
                  ...
                  onChange={(e) => setEditDimensions(prev => ({ ...prev, height: parseFloat(e.target.value) || 0 }))}
                  className="ml-2 w-20 text-black px-1"
                />
              </label>
              <label className="flex items-center">
                ...
                <input
                  ...
                  onChange={(e) => setEditDimensions(prev => ({ ...prev, depth: parseFloat(e.target.value) || 0 }))}
                  className="ml-2 w-20 text-black px-1"
                />
              </label>
              <label className="flex items-center">
